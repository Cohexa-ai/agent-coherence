...
def test_gate_does_not_absolve_a_foreign_edit_it_never_showed_the_caller(
    ...
    between the two (the fixed-stale buffer below) is therefore one the caller
    demonstrably never saw, and the caller's next write must still be denied.
    ...
    agent silently clobbers an edit it was never shown — calling the safety
    check making the NEXT write less safe than skipping it."""
    _seed(tmp_path)
    vol = CoherentVolume(tmp_path, managed=("data/**",), config=fast_cfg)
    try:
        vol.write(REL, b"config-v1")
        ...
        # write-side foreign-edit guard must still fire.
        with pytest.raises(StaleView):
