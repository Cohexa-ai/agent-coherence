...
- the member-classification map covers EXACTLY the 68 ``RegistryBase`` +
...
The expected 68-member set is FROZEN here (imported from the parity test's own
...
# The 68-member expected surface, imported from the parity test's FROZEN
# name-sets (45 base + 13 extended + 9 detection methods + 1 base property). Reusing
...
# ---------------------------------------------------------------------------
# Member classification — exactly the 68 Protocol members, no drift
# ---------------------------------------------------------------------------


def test_member_map_covers_exactly_the_protocol_surface() -> None:
    """The classification map keys equal the 68-member Protocol surface EXACTLY
...
def test_member_map_has_exactly_68_members() -> None:
    ...
    ``last_observed_version_for`` added) + 13 extended methods + 9
    foreign-write detection methods on their own Protocol (the
    uncoverable-run record/read pair added with the work-tree probe) + 1
    base property = 68. Guards against a same-size add+remove that would slip
    past the set-equality check on cardinality alone."""
    assert len(MEMBER_CLASSIFICATION) == 68
    assert len(EXPECTED_MEMBERS) == 68
