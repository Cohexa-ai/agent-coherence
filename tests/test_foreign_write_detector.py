...
def test_classify_is_a_pure_function_of_its_inputs() -> None:
    assert _classify(
        disk_hash="a", canonical_hash="a", updated_at=None,
        has_mediated_writer=False, now_unix=1.0, window_sec=5.0,
    ) == "mediated"
    assert _classify(
        disk_hash="a", canonical_hash="b", updated_at=None,
        has_mediated_writer=False, now_unix=1.0, window_sec=5.0,
    ) == "foreign"
    assert _classify(
        disk_hash="a", canonical_hash="b", updated_at=1.0,
        has_mediated_writer=True, now_unix=2.0, window_sec=5.0,
    ) == "lag_suppressed"
    assert _classify(
        disk_hash="a", canonical_hash="b", updated_at=1.0,
