...
`v0.7` released. See [releases](https://github.com/hipvlady/agent-coherence/releases) for full history. Alpha — APIs may change before `v1.0`.

**What's new in v0.7 — `ccs-diagnose` CLI + supply-chain hardening.**
A passive, zero-network stale-read detector for existing LangGraph graphs. `ccs-diagnose` attaches a passive callback to your compiled graph, classifies its write pattern (`single_writer` / `shared_artifact` / `parallel_branch` / `mixed`), and reports artifacts whose reads were handed divergent versions across nodes. Ships under the `langgraph-v0-preview` classifier with an explicit `v1` promotion gate (≥5 production graphs, ≥3 supervisor topologies, message-trim survival, zero unknown `__`-prefix surprises). Install with `pip install "agent-coherence[diagnose]"`. Output is an HTML report + machine-readable JSON suitable for `jq` and CI pipelines. The release also lands supply-chain hardening: PyPI Trusted Publishers via OIDC, PEP 740 attestations, CycloneDX SBOM attached to every GitHub Release, `requirements-diagnose.txt` hash-pinned for reproducible installs, the rulesets-API-based `ccs-check-release` preflight verifier, and a documented threat model with end-user trust contract at [docs/SECURITY.md](docs/SECURITY.md). End-user docs reorganized: README leads with vendor-neutrality framing, `docs/REPRODUCE.md` carries paper reproduction instructions, `docs/SECURITY.md` carries the public trust contract.

**v0.6 — crash recovery for stale grants.**
