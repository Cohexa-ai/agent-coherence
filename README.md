...
**`v0.10.0` released — foreign-edit coordination across the read and write surfaces, plus the `stale-write-guard-fs` MCP server.** A managed file edited out-of-band (a human, a script, a tool off the coordinator) is now caught at both boundaries: a SHARED holder that re-reads diverged bytes is denied in strict mode (opt-in `CoherentVolume(on_stale_read="raise")`), and `CoherentVolume.write()` denies a write that would clobber a foreign edit **by default** (`on_stale_write="raise"`; content-CAS over the managed path) — both surfaced as `StaleView`, recover via `reacquire()`. New **MCP-C v1**: `stale-write-guard-fs`, a stdio MCP server exposing the coordinator over `swg_read` / `swg_write` / `swg_reacquire` / `swg_write_cas` / `swg_status` with a typed deny contract, path/symlink guards, and fail-closed IO (`pip install agent-coherence[mcp]`). See [CHANGELOG.md](CHANGELOG.md).
