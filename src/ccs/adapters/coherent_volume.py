...
class CoherentVolume:
    ...
    *count*, not the patterns), so a sibling whose globs **differ** from the
    ...
    def _read_impl(
        ...
        # SB-23: seed the foreign-edit baseline only once the caller is about to
        # receive these bytes. A read refused above (StaleView, or a fail-closed
        # CoherenceError) hands the caller nothing; seeding before the refusal
        # would absolve a foreign edit the caller never saw, so a write from its
        # pre-edit buffer would clobber it instead of being denied.
