...
- **The effect gate now holds a write-preempted holder's escaping effect —
  ...
  a stale-status re-read (the warn-mode re-grant a preempted holder gets)
  HOLDs with the new typed `grant_preempted` cause even though both
  comparands are unchanged and confirmed. The strict-mode leg already held
  (the re-read is a deny); this closes the tracked-but-not-strict leg, and
  the epoch's `"write"` exclusion is unchanged — the per-artifact epoch
  cannot see a SHARED holder's preemption and would needlessly fence
  bystanders' CAS reads, so the gate asks the grant-state question directly.
