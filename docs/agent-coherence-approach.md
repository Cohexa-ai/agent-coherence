...
| Escaping side effects fire on a stale input (Sections 2, 5) | `gate()` orders an escaping effect (a deploy, an opened PR, a notification) against the `(version, ownership generation)` pair it was decided from, and raises before firing if either moved — the value changed, or the grant it was read under was reclaimed; it *orders* effects, it never rolls one back; single-host and cooperative |
